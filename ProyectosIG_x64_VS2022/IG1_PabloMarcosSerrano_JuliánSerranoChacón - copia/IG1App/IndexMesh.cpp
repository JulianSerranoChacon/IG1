...
IndexMesh* IndexMesh::generateIndexedOctogone(GLdouble l, GLdouble h)
{//Creamos el mesh a devolver
	IndexMesh* mesh = new IndexMesh();
	GLdouble m = l * 0.5;
	//inicializamos los vertices de la malla
	...
	mesh->vVertices.emplace_back(-m,0, 0);//1
	mesh->vVertices.emplace_back(m, 0, 0);//2
	mesh->vVertices.emplace_back(0, 0, -m);//3
	mesh->vVertices.emplace_back(0, -h, 0);//4
	mesh->vVertices.emplace_back(0, 0, m);//5
...
{//Creamos el mesh a devolver
	...
	mesh->vVertices.reserve(mesh->mNumVertices);
	mesh->vVertices.emplace_back(0, m, 0);//0
	//mesh->vVertices.emplace_back(0, 0, 0);//1
	//mesh->vVertices.emplace_back(m, 0, 0);//2
	//mesh->vVertices.emplace_back(0, 0, m);//3double lado = 360.0 / 3;

	double t = 180.0;
	double lado = 360 / 3;
	for (int i = 0; i < 3; i++) {
		double x = m * glm::cos(glm::radians(t));
		double z = m * glm::sin(glm::radians(t));
		mesh->vVertices.emplace_back(x, 0.0, z);
...
IndexMesh* IndexMesh::generateIndexedPyramidTex(GLdouble l)
{
	IndexMesh* mesh = generateIndexedPyramid(l);
