...
void Scene::setScene(int id) {
	...
	switch (id) {
		...
		case 15:
			...
			break;
		case 16:
			octogone = new IndexedOctogone(300, 300);
			octogone->setMColor(dvec4(1, 0, 0, 1));
			gObjects.push_back(octogone);

			/*pyramid = new IndexedPyramid(100);
			t2 = new Texture();
			t2->load("../bmps/noche.bmp");
			gTextures.push_back(t2);
			pyramid->setTexture(t2);
			pyramid->setMColor(dvec4(1, 0, 0, 1));
			gObjects.push_back(pyramid);*/

			/*semiToroid = new SemiToroid(100, 200, 60, 60);
			semiToroid->setMColor(glm::dvec4(0, 0, 1, 1));
			gObjects.push_back(semiToroid);*/

			//tuberia = new TuberíaCompuesta();
			//gObjects.push_back(tuberia);
			break;
		case 17:
			pyramid = new IndexedPyramid(300);
			//pyramid->setMColor(dvec4(1, 0, 0, 1));
			t2 = new Texture();
			t2->load("../bmps/noche.bmp");
			gTextures.push_back(t2);
